...
SDL_Texture* const ModuleTextures::Load(const char* path)
{
	...
	if (surface == nullptr)
	{
		LOG("The sprite couldnt be loaded, error: %s", SDL_GetError());
		return nullptr;
	}
	SDL_Texture* character = SDL_CreateTextureFromSurface(App->render->renderer, surface);
	...
	textures[last_texture++] = character;
	if (last_texture >= MAX_TEXTURES)
	{
		LOG("Max textures loaded")
			last_texture = 0;
	}
